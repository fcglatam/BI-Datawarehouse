...
inventory_pg.to_sql("xxinventorydaily", con = engine_pg, schema = "public",
    if_exists = "append", index = False)

out_file = path.join(path.join(this_dir, pardir, "data", "history",
    "{}_inventory.csv".format(which_day.strftime("%y%m%d"))))

inventory_pg.to_csv(
    "../data/history/{}_inventory.csv".format(
